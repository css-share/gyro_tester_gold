...
module GyroInputOutputSerializer (
  ...
  input  [31:0] tx1_fifo_data,
  input         tx1_fifo_valid,
  output        tx1_fifo_ready,
  input         tx1_fifo_last,

  input  [31:0] tx2_fifo_data,
  input         tx2_fifo_valid,
  output        tx2_fifo_ready,
  input         tx2_fifo_last,
...
//////////////////////////////////////////////
   ...
   wire tx1_fifo_ready_int;
   wire tx2_fifo_ready_int;
   
   

demux_1_to_4 u_demux_A (
...
demux_1_to_4 u_demux_B (
                       .demux_in(hs_data_out_1_test),
                       .demux_sel(out_channel[3:2]),
                       .demux_out_0(hs_data_out_4_int),
                       .demux_out_1(hs_data_out_5_int),
                       .demux_out_2(hs_data_out_6_int),
                       .demux_out_3(hs_data_out_7_int)
);
...
syncReady u_sync_ready_1 (
    .clk(~clock),  
    .rst_n(reset_n), 
    .next(out_next_int), 
    .ready(tx1_fifo_ready_int)
  );   

syncReady u_sync_ready_2 (
    .clk(~clock),  
    .rst_n(reset_n), 
    .next(out_next_int), 
    .ready(tx2_fifo_ready_int)
  );   
...
outputShiftRegister32Bits OUT_SHIFT_REG_1(
    .shift(outSR_shift),
    .reset_n(reset_n),
    .load(tx1_fifo_ready_int & tx1_fifo_valid),
    .d_in(tx1_fifo_data), 
...
outputShiftRegister32Bits OUT_SHIFT_REG_2(
    .shift(outSR_shift),
    .reset_n(reset_n),
    .load(tx2_fifo_ready_int & tx2_fifo_valid),
    .d_in(tx2_fifo_data), 
  ...
  assign HSI_DBP    = (hs_data_out_4_int & enable & hsi_enable);
  assign HSI_DBM    = (hs_data_out_5_int & enable & hsi_enable);
  assign HSI_DC     = (hs_data_out_2_test & enable & hsi_enable);
  assign MCK        = (clock_div_2 & enable);



  assign tx_fifo_ready   = tx_fifo_ready_int;
  assign tx1_fifo_ready  = tx1_fifo_ready_int;
  assign tx2_fifo_ready  = tx2_fifo_ready_int;
